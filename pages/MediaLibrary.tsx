    ...
    <Layout title="Library" showBack backPath={isKid ? "/kids" : "/hub"} isKidsMode={isKid} hideBottomNav={isKid}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {activeTab === 'books' ? LIBRARY_BOOKS.map((book, i) => (
          <button key={i} onClick={() => setReadingBook(book)} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 flex gap-4 text-left hover:shadow-lg transition-all group items-center">
             <div className="w-24 aspect-[3/4] bg-gray-100 rounded-lg overflow-hidden relative shrink-0">
               ...
               <div className="absolute inset-0 bg-black/10 group-hover:bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                   <Book className="text-white drop-shadow-md" size={24} />
               </div>
             </div>
             <div>
               <h4 className="font-bold text-gray-800 text-base leading-tight mb-2">{book.title}</h4>
               <span className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded font-medium">{book.type}</span>
             </div>
          </button>
        )) : WORKBOOKS.map((wb) => (
          <button key={wb.id} onClick={() => navigate(`/library/workbook/${wb.id}`)} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 flex gap-4 text-left hover:shadow-md transition-all items-center">
             <div className="w-24 aspect-[3/4] bg-gray-100 rounded-lg overflow-hidden relative shrink-0">
               <img src={wb.cover} className="w-full h-full object-cover" alt={wb.title}/>
               <div className="absolute top-2 right-2 bg-yellow-400 text-yellow-900 text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm">NEW</div>
             </div>
             <div>
               <h4 className="font-bold text-gray-800 text-base leading-tight mb-2">{wb.title}</h4>
      ...
      {readingBook && createPortal(
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-white rounded-3xl w-full max-w-2xl h-[85vh] flex flex-col overflow-hidden animate-in zoom-in duration-300 relative">
                  ...
                  <div className="bg-orange-50 p-6 border-b border-orange-100">
                      <h2 className="text-xl md:text-2xl font-bold text-gray-800 pr-8">{readingBook.title}</h2>
                      <p className="text-gray-500 text-sm">Interactive Story</p>
                  </div>

                  <div className="flex-1 overflow-y-auto p-6 md:p-8 font-serif text-lg leading-relaxed text-gray-700">
                      <div className="float-left w-1/3 mr-4 mb-2 rounded-xl overflow-hidden shadow-md">
                  ...
                  <div className="p-4 border-t border-gray-100 flex justify-between bg-gray-50 pb-[max(1rem,env(safe-area-inset-bottom))]">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         {VIDEO_RESOURCES.map((v) => (
           <button onClick={() => setPlayingVideo(v)} key={v.id} className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex gap-4 text-left hover:shadow-md transition-all group">
             <div className="w-32 aspect-video bg-gray-200 rounded-lg relative overflow-hidden shrink-0">
