                    ...
                    {!flipped ? (
                        <>
                            <Image
                                ...
                                resizeMode="contain"
                            />
                            <Text className="text-5xl font-kids font-bold text-gray-800 text-center">{currentWord.igbo}</Text>
                            <Text className="text-gray-400 mt-4 font-bold">Tap to flip</Text>
                        </>
                    ) : (
                        <>
                             <Text className="text-4xl font-kids font-bold text-pink-500 text-center mb-4">{currentWord.english}</Text>
                             <Text className="text-gray-400 font-bold">Tap to flip back</Text>
                        </>
                    )}
                </TouchableOpacity>

                <View className="flex-row gap-6">
                    <TouchableOpacity onPress={playAudio} className="bg-blue-400 p-5 rounded-full border-b-4 border-blue-600 active:border-b-0 active:translate-y-1 shadow-lg">
                        <Volume2 size={32} color="white" />
                    </TouchableOpacity>

                    <TouchableOpacity onPress={nextCard} className="bg-green-400 p-5 rounded-full border-b-4 border-green-600 active:border-b-0 active:translate-y-1 shadow-lg">
