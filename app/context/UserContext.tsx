...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadProfiles = async () => {
    try {
      const saved = await AsyncStorage.getItem('lingbo_profiles');
  ...
  useEffect(() => {
    AsyncStorage.setItem('lingbo_profiles', JSON.stringify(profiles));
  }, [profiles]);

  useEffect(() => {
    if (activeProfileId) {
      AsyncStorage.setItem('lingbo_active_profile_id', activeProfileId);
    } else {
      AsyncStorage.removeItem('lingbo_active_profile_id');
    }
  }, [activeProfileId]);
