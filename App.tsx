...
const MemoryGame = ({ onBack }: { onBack: () => void }) => {
  ...
  const handleCardClick = (index: number) => {
    ...
    if (newFlipped.length === 2) {
      ...
      if (id1 === id2) { setTimeout(() => { playGameSound('success'); setMatched(p => [...p, id1]); setFlipped([]); if (matched.length + 1 === MEMORY_GAME_DATA.length/2) { playGameSound('win'); setWon(true); } }, 300); } 
  ...
  return (
    <div className="bg-white rounded-3xl p-4 md:p-6 shadow-xl border-4 border-purple-100 min-h-[500px] flex flex-col relative overflow-hidden">
      ...
      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 flex-1 content-start">
        {cards.map((card, i) => (
          <button key={card.uid} onClick={() => handleCardClick(i)} className="aspect-square relative group cursor-pointer" style={{ transformStyle: 'preserve-3d' }}>
            <div className="w-full h-full transition-all duration-500" style={{ transformStyle: 'preserve-3d', transform: (flipped.includes(i) || matched.includes(card.matchId)) ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
               <div className="absolute inset-0 bg-gradient-to-br from-purple-400 to-purple-600 rounded-xl flex items-center justify-center shadow-sm" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}><span className="text-white font-kids font-bold text-4xl opacity-50">?</span></div>
               <div className="absolute inset-0 bg-white rounded-xl border-4 border-purple-200 flex items-center justify-center overflow-hidden shadow-sm" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>{card.type === 'image' ? <img src={card.content} className="w-full h-full object-cover" /> : <span className="font-kids font-bold text-gray-800 text-sm text-center px-1">{card.content}</span>}</div>
            </div>
          </button>
        ))}
      </div>
...
const SpeedTapGame = ({ onBack }: { onBack: () => void }) => {
  ...
  useEffect(() => {
    if (isPlaying && timeLeft > 0) { const timer = setInterval(() => setTimeLeft(p => p - 1), 1000); return () => clearInterval(timer); } 
    else if (timeLeft === 0) { setIsPlaying(false); setGameOver(true); playGameSound('win'); }
  }, [isPlaying, timeLeft]);

  const generateQuestion = () => {
    const card = KIDS_FLASHCARDS[Math.floor(Math.random() * KIDS_FLASHCARDS.length)];
    let options = [card]; while (options.length < 3) { const r = KIDS_FLASHCARDS[Math.floor(Math.random() * KIDS_FLASHCARDS.length)]; if (!options.find(o => o.igbo === r.igbo)) options.push(r); }
    setCurrentQuestion({ target: card, options: options.sort(() => Math.random() - 0.5) });
  };
  const startGame = () => { setScore(0); setTimeLeft(30); setIsPlaying(true); setGameOver(false); generateQuestion(); };
  const handleAnswer = (option: any) => {
    if (option.igbo === currentQuestion.target.igbo) {
      playGameSound('success'); setScore(s => s + 10); setFeedback('correct');
      setTimeout(() => { setFeedback(null); generateQuestion(); }, 200);
    } else {
      playGameSound('error'); setTimeLeft(t => Math.max(0, t - 2)); setFeedback('wrong');
      setTimeout(() => setFeedback(null), 200);
    }
  };

  return (
    <div className={`bg-white rounded-3xl p-6 shadow-xl border-4 min-h-[500px] flex flex-col relative overflow-hidden text-center transition-colors ${feedback === 'correct' ? 'border-green-400 bg-green-50' : feedback === 'wrong' ? 'border-red-400 bg-red-50' : 'border-blue-100'}`}>
      <div className="flex justify-between items-center mb-6"><button onClick={onBack}><ArrowLeft size={24} className="text-gray-600" /></button><div className="flex gap-4"><div className="bg-blue-100 text-blue-600 px-4 py-1 rounded-full font-bold flex items-center gap-2"><Trophy size={16}/> {score}</div><div className={`px-4 py-1 rounded-full font-bold flex items-center gap-2 ${timeLeft < 10 ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-green-100 text-green-600'}`}><Timer size={16}/> {timeLeft}s</div></div><div className="w-6"></div></div>
      {!isPlaying && !gameOver && (<div className="flex-1 flex flex-col items-center justify-center animate-in zoom-in"><h3 className="font-kids text-3xl font-bold text-blue-600 mb-4">Speed Tap!</h3><p className="text-gray-500 mb-8">Tap the correct picture before time runs out.</p><button onClick={startGame} className="bg-blue-500 text-white font-bold text-xl px-12 py-4 rounded-full shadow-lg hover:bg-blue-600 hover:scale-105 transition-all">Start Game</button></div>)}
      {isPlaying && currentQuestion && (<div className="flex-1 flex flex-col animate-in fade-in"><div className="flex-1 flex items-center justify-center mb-6"><h2 className="font-kids text-5xl font-bold text-gray-800">{currentQuestion.target.igbo}</h2></div><div className="grid grid-cols-3 gap-4">{currentQuestion.options.map((opt: any, i: number) => (<button key={i} onClick={() => handleAnswer(opt)} className="aspect-square bg-white rounded-2xl border-2 border-gray-100 p-2 hover:border-blue-400 active:scale-95 transition-all shadow-sm"><img src={opt.image} className="w-full h-full object-contain" /></button>))}</div></div>)}
      {gameOver && (<div className="absolute inset-0 z-50 bg-white/95 flex flex-col items-center justify-center animate-in zoom-in backdrop-blur-sm"><Trophy size={80} className="text-yellow-400 mb-4" /><h3 className="font-kids text-4xl font-bold text-gray-800 mb-2">Game Over!</h3><p className="text-xl text-gray-500 mb-8">Final Score: <span className="font-bold text-blue-600">{score}</span></p><button onClick={startGame} className="bg-blue-500 text-white font-bold px-8 py-3 rounded-full shadow-lg hover:bg-blue-600">Play Again</button></div>)}
    </div>
  );
};
